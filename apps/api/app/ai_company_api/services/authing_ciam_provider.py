...
SUPPORTED_TOKEN_AUTH_METHOD = "client_secret_basic"
MANAGEMENT_TOKEN_EXPIRY_SKEW = timedelta(seconds=30)


@dataclass(frozen=True)
class AuthingCiamConfig:
    app_host: str
    issuer: str
    client_id: str
    app_secret: str = field(repr=False)
    user_pool_id: str
    user_pool_secret: str = field(repr=False)
    management_api_base_url: str = "https://api.authing.cn"
    request_timeout_seconds: float = 5.0
    clock_skew_seconds: int = 60
    jwks_cache_seconds: int = 300

    def __post_init__(self) -> None:
        app_host = self.app_host.rstrip("/")
        issuer = self.issuer.rstrip("/")
        management_api_base_url = self.management_api_base_url.rstrip(
            "/"
        )
        if (
            not _is_https_url(app_host)
            or urlparse(app_host).path not in {"", "/"}
            or not _is_https_url(issuer)
            or issuer != f"{app_host}/oidc"
            ...
            raise ValueError("Authing CIAM configuration is not valid")
...
class AuthingCustomerIdentityProvider:
    def __init__(
        self,
        config: AuthingCiamConfig,
        *,
        ...
        self._logout_hint_cipher = Fernet(
            urlsafe_b64encode(
                sha256(
                    (
                        "ai-scdc-authing-logout-hint-v1\0"
                        f"{config.issuer}\0{config.client_id}\0"
                        f"{config.app_secret}"
                    ).encode("utf-8")
                ).digest()
            )
    ...
    def exchange_code(
        ...
        *,
        ...
        payload = self._request_json(
            ...
            auth=(self.client_id, self._config.app_secret),
        ...
        return OidcTokenResponse(
            ...
            logout_hint=(
                self._logout_hint_cipher.encrypt(
    ...
    def end_session_url(
        ...
        *,
        ...
        if logout_hint is not None:
            try:
                id_token_hint = self._logout_hint_cipher.decrypt(
                    logout_hint.encode("ascii")
                ).decode("utf-8")
            except (InvalidToken, UnicodeError):
                raise CustomerIdentityProviderError(
                    "CIAM logout hint is not valid"
                ) from None
    ...
    def identity_status(
        ...
        *,
        ...
        for attempt in range(2):
            ...
            _raise_for_management_json_unavailability(
                payload,
                operation="identity status",
            )
            unauthorized = payload.get("statusCode") == 401
    ...
    def _validate_discovery(
        ...
        if (
            ...
            or not all(
                ...
                and _same_origin(self._config.app_host, value)
            ...
            or SUPPORTED_TOKEN_AUTH_METHOD
            not in _string_list(
                payload.get(
    ...
    def _validate_time_claims(
        ...
        raw_not_before = claims.get("nbf")
        not_before = (
            _numeric_date(raw_not_before)
        ...
        *,
        ...
        with self._cache_lock:
            ...
            payload = self._request_json(
                ...
                json={
                    "accessKeyId": self._config.user_pool_id,
                    "accessKeySecret": (
                        self._config.user_pool_secret
                    ),
                },
    ...
    def _status_from_payload(
        ...
        *,
        ...
        if (
            payload.get("statusCode") != 200
            or not isinstance(data, dict)
            or data.get("userId") != expected_subject
        ):
            raise CustomerIdentityProviderError(
...
def _raise_for_management_json_unavailability(
    payload: dict[str, Any],
    *,
    operation: str,
) -> None:
...
def _is_authing_user_pool_id(value: str) -> bool:
    return (
        len(value) == 24
